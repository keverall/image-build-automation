...
CI pipeline stage. Does **not** duplicate Pester testing guidance
(see [`powershell_testing.md`](powershell_testing.md)).
...
pipeline. The `environment {}` block no longer uses `credentials()`
...
environment variables for all subsequent stages.
...
## 2. Pipeline — Standalone PowerShell Stage

The existing pipeline runs on `label 'windows'`. A dedicated PS stage can be
dropped in alongside the existing `Unit Tests & Coverage` stage — no new
...
stage('PowerShell — Pester Unit Tests') {
    ...
    steps {
        powershell '''
            ...
            # Run unit tests, emit JUnit XML for CI
