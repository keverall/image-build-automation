...
Queries GET /rest/server-hardware across all pages and returns a normalised list of servers (name, serial, model, power state, health, iLO IP, enclosure). Each server also reports its HPE OneView maintenance mode in the `MaintMode` column as `Yes` or `No`, derived from the server-hardware resource's maintenance state fields. Supports an optional -Filter to narrow the result by health, power state, maintenance mode, or name (substring/wildcard match).
...
.SYNOPSIS
    ...
    .DESCRIPTION
        ...
        as `Yes` or `No` in the `MaintMode` column.
