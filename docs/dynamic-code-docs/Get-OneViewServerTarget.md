...
Sends a query against the OneView /rest/server-hardware endpoint and returns a normalized hashtable describing the server.  Validates health (must be OK) and tolerates power state Off or On. Reports the server's maintenance-mode state (Yes/No) in `Details.maintenance_mode`. The HPEOneView module's Get-OVServer is the preferred source because the REST endpoints do not reliably expose maintenance state; the REST payload's own maintenance fields are used as a fallback when the module is unavailable. STRICT SINGLE-SERVER: this command must resolve to exactly one server. A query that matches more than one server is a hard failure (Success=$false) rather than a warning - it never silently picks the first match, because it underpins destructive operations (ISO attach/deploy, reboot, OS build). Connection to the appliance is handled by the shared Resolve-OneViewSession helper (prompts for the host/credentials when needed) and the session persists; this command never disconnects.
