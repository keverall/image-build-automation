...
| `-RepoBaseUrl` | HTTPS base URL of the ISO repository (only used when hosting ISOs on an HTTPS repo; otherwise supply the ISO directly from a network share via -ExternalIsoPath). |
