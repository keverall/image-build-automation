...
3. Configuration Manager bootable media ISO must be held on a network share  
   1. which by definition is windows shared file  
   2. and has an SMB address provided or a networkshare name and path/filename,  
...
3. Store the ISO in a on a network share,  
   1. which by definition is windows shared,  
   2. and has an SMB address provided or a networkshare name and path/filename,  
   3. and that network share exists on the windows server  
   4. which image-build-automation (this repo is being run from)  
   5. that network path/smb address is reachable by iLO virtual media.  
