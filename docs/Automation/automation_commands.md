...
- [Built-in command help (`-Help`)](#built-in-command-help-help)
...
<a id="built-in-command-help-help"></a>

## Built-in command help (`-Help`)

Every main `Automation` command (the exported cmdlets, not the private `_`-prefixed helpers) accepts a `-Help` switch that prints a man-like reference panel for that command — no need for `Get-Help`. The reference is generated by the module's own `Get-CommandHelp` function (in `Private/Help.ps1`) and shows:

- The command's synopsis
- Every parameter, grouped as **Mandatory** vs **Optional**
- For each parameter: its type, whether it accepts pipeline input, its default value, and any **allowed values** (ValidateSet) or regex constraint (ValidatePattern)
- The resolved command source file and the GitHub link to it

```powershell
# Print the full parameter reference for a single command
Get-OneViewServerList -Help

# Works with any main command
Set-MaintenanceMode -Help
Configure-PhysicalBuild -Help
```
...
SYNOPSIS
    List all servers connected to HPE OneView.
...
PARAMETERS
  ...
  OPTIONAL
    -Environment <string>
        Allowed: Prod, DR, Lab, Dev
        Default: Prod
    -ManagementHost <string>
        The OneView appliance hostname or IP.
    -Help [<switch>]
        Show this command reference and exit.

SOURCE
    src/powershell/Automation/Public/Get-OneViewServerList.ps1
    https://github.com/.../blob/main/src/powershell/Automation/Public/Get-OneViewServerList.ps1
```
