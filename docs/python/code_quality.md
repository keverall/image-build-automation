...
Validates Python syntax for all scripts (including `src/python/automation/utils/*.py`) and JSON configuration files.

### 2. Code Quality & Security Scan (new)

This stage runs immediately after Setup and before any build/deploy operations. It:
- Lints all Python code (`src/python/automation/`))
...
**Command**:
```bash
ruff check src/python/automation/ --format=json --output=ruff_issues.json
ruff format --check src/python/automation/ --output=ruff_format.txt
```
...
**Command**:
```bash
pylint --output-format=json src/python/automation/ > pylint_report.json
pylint src/python/automation/ > pylint_report.txt
```
...
**Example**:
```
src/python/automation/cli/build_iso.py - A (54.36)
src/python/automation/cli/maintenance_mode.py - A (19.16)
```
...
**Command**:
```bash
bandit -r src/python/automation/ -f json -o bandit_report.json
```
...
# Lint + format check
ruff check src/python/automation/ --fix
ruff format src/python/automation/

# Complexity
radon mi src/python/automation/ -s
radon cc src/python/automation/ -nc

# Security
bandit -r src/python/automation/
safety check
