...
Maintenance mode manages scheduled maintenance windows for clusters across three monitoring systems:
...
- HPE OpenView: Enable maintenance mode for cluster of servers or disable or stop separately.
...
1. **Enable** — validate cluster ID, optionally compute end time from the cluster schedule, then enable maintenance on each configured subsystem (SCOM, iLO, OpenView). Optionally schedule a one-shot task to run disable at end time.
...
Every run writes a timestamped JSON file and appends one JSON line to a master log. Records include cluster ID, action, dry-run flag, per-system success flags (SCOM, iLO, OpenView, email, OpsRamp), start/end timestamps, and any errors.
...
| OpenView not implemented in initial release | Set `"use_cli": true` and `"cli_path"` in `openview_config.json` for CLI fallback |
