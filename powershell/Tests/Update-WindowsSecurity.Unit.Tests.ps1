...
# Dedicated unit tests for the Update-WindowsSecurity public function.
...
Describe 'Update-WindowsSecurity — basic invocation and parameter validation' {
    It 'Function is exported and has expected parameters' {
        $cmd = Get-Command Update-WindowsSecurity -ErrorAction SilentlyContinue
    ...
    It 'Accepts -DryRun switch without throwing' {
        ...
        { & Update-WindowsSecurity -DryRun -ErrorAction SilentlyContinue } | Should -Not -Throw
    }

    It 'Rejects unknown parameters (strict mode)' {
        { & Update-WindowsSecurity -NonExistentParam 2>&1 } | Should -Not -Be $null
    }
}
