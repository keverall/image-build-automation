...
def _cyberark_fetch(safe_name: str, object_name: str, app_id: str = "jenkins") -> Optional[tuple[str, str]]:
    ...
    for cli_name in ("ark_ccl", "ark_cc", "CyberArk.CLI"):
        try:
            result = subprocess.run(
                [cli_name, "getpassword", f"-pAppID={app_id}", f"-pSafe={safe_name}", f"-pObject={object_name}"],
                capture_output=True,
                text=True,
                timeout=15,
            )
    ...
    # ── Method 2: REST API ───────────────────────────────────────────────────
    aim_url = (
        os.environ.get("AIM_WEBSERVICE_URL")
...
def _resolve(
    var_name: str, safe_name: str, object_name: str, default: Optional[str] = None, required: bool = False
) -> str:
    ...
    if required:
        raise ValueError(
            ...
            f"(expected in CyberArk safe={safe_name}, object={object_name})."
        )
    return None
...
def get_credential(env_var_name: str, default: Optional[str] = None, required: bool = False) -> Optional[str]:
    ...
    """
    return _resolve(env_var_name, safe_name="Jenkins", object_name=env_var_name, default=default, required=required)
...
def get_ilo_credentials(
    ...
    username = _resolve(
        username_env, safe_name="HPE-iLO", object_name=username_env, default=default_username, required=False
    )
    password = _resolve(
        password_env, safe_name="HPE-iLO", object_name=password_env, default=default_password, required=False
    )
...
def get_scom_credentials(
    ...
    username = _resolve(username_env, safe_name="SCOM-2015", object_name=username_env, required=True)
    password = _resolve(password_env, safe_name="SCOM-2015", object_name=password_env, required=True)
    return username, password
...
def get_openview_credentials(
    ...
    return (
        _resolve(user_env, safe_name="OpenView", object_name=user_env, default=None),
        _resolve(pass_env, safe_name="OpenView", object_name=pass_env, default=None),
    )
...
def get_smtp_credentials(
    ...
    return (
        _resolve(user_env, safe_name="SMTP-Mail", object_name=user_env, default=None),
        _resolve(pass_env, safe_name="SMTP-Mail", object_name=pass_env, default=None),
    )
