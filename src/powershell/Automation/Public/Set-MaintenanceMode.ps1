...
function Set-MaintenanceMode {
    ...
    param(
        ...
        [Alias('TgtId')]
        [Parameter(Position = 1)][string] $TargetId,
        [Parameter(Mandatory, ParameterSetName = 'Run', Position = 2)][ValidateSet('scom', 'oneview')][string] $Mode,
        [Alias('Env')]
        [ValidateSet('Test', 'Prod')][string] $Environment,
        [Alias('OVHost')]
        [string] $OneViewHost,
    ...
    #
    # For OneView mode, reuse an existing active session when one is present. This
    # avoids re-prompting for credentials (and opening a duplicate REST session) when
    # the operator already connected via Connect-OneView. The OneViewClient script
    # block (generated by the module) also checks for an existing session and will
    # reuse it without credentials, so we only prompt when there is genuinely no
    # active session to reuse.
    $hasActiveOneViewSession = $false
    if ($Mode -eq 'oneview' -and -not $DryRun) {
        try { $hasActiveOneViewSession = Test-OneViewSessionActive -ErrorAction SilentlyContinue } catch { }
    }

    if ($DryRun) {
        $resolvedUsername = $null
        $resolvedPassword = $null
    } elseif ($Mode -eq 'oneview' -and $hasActiveOneViewSession) {
        # Existing OneView session is active — skip credential resolution.
        # OneViewClient will reuse the session in its generated script block.
        $resolvedUsername = $null
        $resolvedPassword = $null
        Write-Verbose "Reusing active OneView session — skipping credential prompt."
    } else {
    ...
    if (-not $DryRun) {
        ...
        if ($Mode -eq 'oneview' -and $oneviewMgr) {
            if ($hasActiveOneViewSession) {
                Write-Verbose "Reusing existing OneView session to '$($oneviewMgr.Appliance)' — skipping connection test."
            } else {
    ...
    if ($Action -eq 'enable') {
        ...
        if ($opsrampClient -and -not $DryRun) {
            ...
            foreach ($s in $servers) {
                $opsrampClient.SendMetric($s, 'maintenance.mode', 1, @{ cluster = $TargetId; environment = $env }, [DateTime]::MinValue)
            }
        ...
        if ($opsrampClient -and -not $DryRun) {
            ...
            foreach ($s in $servers) {
                $opsrampClient.SendMetric($s, 'maintenance.mode', 0, @{ cluster = $TargetId; environment = $env }, [DateTime]::MinValue)
            }
...
if (`$server) {
    `$result = @{ Success = `$true; TargetType = 'ServerHardware'; TargetName = `$server.Name; SerialNumber = `$server.serialNumber; MaintenanceModeEnabled = [bool](`$server.maintenanceMode -and `$server.maintenanceMode -notin @('Off', `$false, `$null)); Model = `$server.model; State = `$server.state; Message = 'Found server' }
    `$result | ConvertTo-Json -Depth 5
    return
}
...
if ('$TargetType' -eq 'ServerHardware') {
    ...
    `$obj = @{
        ...
        Status = if (`$inMaint) { 'in_maintenance' } else { 'not_in_maintenance' }
        Message = if (`$inMaint) { 'Server is in maintenance mode' } else { 'Server is not in maintenance mode' }
    }
    if (`$inMaint) {
        `$inMaintenance++
    } else {
