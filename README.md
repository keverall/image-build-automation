...
| [⚙️ Code Quality & Security](docs/Generic/code_quality.md#top) | PSScriptAnalyzer, Ruff (Python), gitleaks configuration |
...
This project is Makefile-driven. The canonical way to run day-to-day tasks is:
...
| `make test` | Run the full Pester v6 test suite (verbose) |
| `make test-unit` | Run only unit tests |
| `make test-integration` | Run only integration tests |
| `make coverage` | Run tests with code coverage → `generated/coverage/` |
| `make docs` | Generate the PowerShell API docs (`docs/dynamic-code-docs/`), fix links, and add anchors/TOC |
| `make fix-docs` | Repair broken markdown links and regenerate anchors/TOC across `configs/`, `docs/`, and root markdown |
| `make gen-docs` | Regenerate `docs/dynamic-code-docs/` from `src/` and `scripts/` source |
| `make add-anchors` | Add Bitbucket/GitStash-compatible anchors + TOC to all markdown files |
| `make fix-docs-dryrun` | Preview link/anchor/TOC fixes without writing |
| `make word-docs` | Convert Markdown docs to Word DOCX with clickable bookmarks/links |
| `make rtf-docs` | Convert Markdown docs to RTF |
| `make prune-logs` | Prune log files older than 30 days |
| `make clean` | Remove build artifacts and temp files |
| `make ci` | Full pipeline: lint + coverage |
| `make all` | Run linting and tests |

> **Note — Python linting:** `make lint-python` requires Ruff, which is resolved
> automatically via `uv run ruff` if `uv` is on `PATH`, otherwise via
> `python -m ruff`, or a one-shot `pip install --user ruff`. Configuration lives
> in [`ruff.toml`](ruff.toml) at the repository root.
