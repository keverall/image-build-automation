...
& "$env:LOCALAPPDATA\Programs\oh-my-posh\bin\oh-my-posh.exe" init pwsh --config "C:\Users\98253\Documents\WindowsPowerShell\pwsh10k.omp.json" | Invoke-Expression



# oh-my-posh --init --shell pwsh --config ~/pwsh10k.omp.json | Invoke-Expression
...
# Source - https://stackoverflow.com/a/38375707
# Posted by Jeremy Danyow, modified by community. See post 'Timeline' for change history
...
Set-Alias ls ezals -Option AllScope -Force
...
# Set-Alias repos 'Set-Location /mnt/wsl/Ubuntu-25.10-Dev/home/kev/repos' && builtin cd -
...
function gcm {
    ...
    Notepad does not have a built-in feature to display line numbers. However, you can enable line numbers in more advanced text editors like Notepad++ or Visual Studio Code. If you need line numbers for coding or editing, consider using one of these alternatives.
...
# Source - https://stackoverflow.com/a/79279479
...
GetFile('C:\Program Files\Notepad++\notepad++.exe').ShortPath.
Replace('\', '/')
...
# Source - https://stackoverflow.com/a/56366936
# Posted by Matt McNabb
# Retrieved 2025-12-14, License - CC BY-SA 4.0

Set-Location $env:APPDATA

# Network setup removed as it may be causing slowdowns

# Set to local directory for pyenv setup to avoid UNC path issues
...
# Wrapper function for pyenv to avoid UNC path issues
function pyenv {
    Push-Location $env:USERPROFILE
    try {
        $pyenvBat = "$env:USERPROFILE\.pyenv\pyenv-win\bin\pyenv.bat"
        & $pyenvBat $args
    }
    finally {
        Pop-Location
