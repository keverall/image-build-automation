...
### 47) `Get-OneViewServerList` Detail table rendering fix — single-record rows + inline-if PS 7.0 compatibility
...
<a name="root-cause-47"></a>

#### Root cause

- **Inline `if` expression as method argument (PS 7.0 incompatibility).** On the `cellColors.Add(...)` line, the expression was written as `cellColors.Add(if ($srv.maintenance_mode -eq 'Yes') { 'Red' } else { 'Green' })`. The `if (...) { ... } else { ... }` *expression* syntax used as a bare argument was introduced in PowerShell 7.1; on PowerShell 7.0.6 the parser cannot bind it, so the statement fails and every server-row test throws — producing 19 failures across the `Get-OneViewServerList` unit tests.
- **Per-cell `Write-Host -NoNewline` fragmented rows into multiple Information records.** `_Format-OneViewServerListResult` emitted each data row as a sequence of `Write-Host ' ' -NoNewline`, `Write-Host $cellText -NoNewline`, `Write-Host (' ' * $padLen) -NoNewline`, then a final `Write-Host ' |'`. Each `Write-Host` call creates a separate Information-stream record, so a 12-column data row spanned 24+ records. The test suite's column-count assertion (`$hostRecords | Where-Object … | Split-PipeCount`) expected one record per row; the fragmented records produced mismatched counts on the `-Detail Model` and `-Detail ROM` tests (17 of the 19 failures were the inline-`if`; the remaining 2 were the row-fragmentation).

<a name="fix-47"></a>

#### Fix

- **`Get-OneViewServerList.ps1` (line ~534):** replaced `cellColors.Add(if ($srv.maintenance_mode -eq 'Yes') { 'Red' } else { 'Green' })` with `$maintColor = if ($srv.maintenance_mode -eq 'Yes') { 'Red' } else { 'Green' }` followed by `cellColors.Add($maintColor)`, restoring PowerShell 5.1 / 7.0 compatibility.
- **`Get-OneViewServerList.ps1` (lines ~551–563):** replaced the per-cell `for` loop of `Write-Host -NoNewline` calls with a single `$rowParts` accumulation that builds the full pipe-delimited row text (`"| $($rowParts -join '| ')|"`) and emits it via one `Write-Host $rowText -ForegroundColor $rowColor` call per data row. This preserves the visual colour (row base = health colour, Red override when in maintenance) and the pipe-delimited structure the tests assert, while producing exactly one Information stream record per row.
- **`recent-changes.md`:** removed terminal-capture pollution appended by commit `d40583ba` (interactive `Connect-OneView` session output including credential echo), restoring the file to its authored changelog content.

<a name="verification-47"></a>

#### Verification

- `Get-OneViewServerList.Unit.Tests.ps1` → **25 passed, 0 failed** (was 19 failures).
- `make test-unit` → **582 passed, 0 failed**.
- `make test` → **582 passed, 0 failed** (0 failed; the earlier `make test` 19-failure run was resolved by this fix).
- `make lint` (PSScriptAnalyzer) → **153 files, all checks passed**.
- PowerShell parser: `Get-OneViewServerList.ps1` parses with zero syntax errors on PowerShell 7.0.6.
