...
Describe 'New-OneViewMaintenanceScript — basic invocation and parameter validation' {
    ...
    It 'Returns a string script block for enable operation' {
        $result = New-OneViewMaintenanceScript -Appliance 'oneview.example.com' -ScopeName 'TestScope' -Operation 'enable'
        $result | Should -BeOfType [string]
        $result | Should -Match 'Import-Module HPOneView.Managed'
        $result | Should -Match 'Connect-OVMgmt'
        $result | Should -Match 'Get-OVScope'
        $result | Should -Match 'Enable-OVMaintenanceMode'
    }

    It 'Returns a string script block for disable operation' {
        $result = New-OneViewMaintenanceScript -Appliance 'oneview.example.com' -ScopeName 'TestScope' -Operation 'disable'
        $result | Should -BeOfType [string]
        $result | Should -Match 'Import-Module HPOneView.Managed'
    ...
    It 'Includes Async parameter when Async is true' {
        $result = New-OneViewMaintenanceScript -Appliance 'oneview.example.com' -ScopeName 'TestScope' -Operation 'enable' -Async $true
