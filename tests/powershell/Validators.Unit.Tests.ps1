...
Describe 'Test-ClusterId' {
    It 'Returns definition for a valid cluster ID' {
        $result = Test-ClusterId -ClusterId 'TEST-CLUSTER' -CataloguePath (Join-Path $Script:ConfigDir 'clusters_catalogue.json')
    ...
    It 'Returns failure for a missing catalogue file' {
        $result = Test-ClusterId -ClusterId 'X' -CataloguePath 'C:\nonexistent.json'
        $result.Success | Should -Be $false
        $result.Error | Should -Match 'not found'
    }

    It 'Returns failure for a cluster not in the catalogue' {
        $result = Test-ClusterId -ClusterId 'NONEXISTENT-CLUSTER' -CataloguePath (Join-Path $Script:ConfigDir 'clusters_catalogue.json')
        $result.Success | Should -Be $false
        $result.Error | Should -Match 'not found'
    }
}
